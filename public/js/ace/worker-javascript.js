  ...
  var JSHINT = (function () {
    ...
    function comma() {
      if (token.line !== nexttoken.line) {
        ...
        !token.comment &&
        token.character !== nexttoken.from &&
        option.white
      ) {
      ...
      nud: function () {
        ...
        if (funct === s) {
          ...
          if (option.undef && typeof predefined[v] !== "boolean") {
            if (
              !(anonname === "typeof" || anonname === "delete") ||
              (nexttoken &&
    ...
    infix(
      ...
      function (left, that) {
        ...
        if (
          ...
          else if (directive["use strict"]) error("Strict violation.");
        } else if (
          !option.evil &&
          left &&
          left.value === "document" &&
        ...
        if (left) {
          if (left.type === "(identifier)") {
            if (left.value.match(/^[A-Z]([A-Z0-9_$]*[a-z][A-Za-z0-9_$]*)?$/)) {
              if (
                "Number String Boolean Date Object".indexOf(left.value) === -1
              ) {
                if (left.value === "Math") {
    ...
    prefix("(", function () {
      ...
      if (option.immed && v.id === "function") {
        if (
          nexttoken.id !== "(" &&
          (nexttoken.id !== "." ||
            (peek().value !== "call" && peek().value !== "apply"))
        ) {
          warning(
    ...
    function createMetrics(functionStartToken) {
      return {
        ...
        verifyMaxStatementsPerFunction: function () {
          if (
            option.maxstatements &&
            this.statementCount > option.maxstatements
          ) {
            var message =
              "Too many statements per function (" + this.statementCount + ").";
        ...
        verifyMaxParametersPerFunction: function (params) {
          ...
          if (option.maxparams && params.length > option.maxparams) {
            var message =
              "Too many parameters per function (" + params.length + ").";
            warning(message, functionStartToken);
          }
        },
        ...
        verifyMaxNestedBlockDepthPerFunction: function () {
          if (
            ...
            var message =
              "Blocks are nested too deeply (" + this.nestedBlockDepth + ").";
            warning(message);
          }
        },
        ...
        verifyMaxComplexityPerFunction: function () {
          ...
          if (max && cc > max) {
            var message =
              "Cyclomatic complexity is too high per function (" + cc + ").";
            warning(message, functionStartToken);
          }
        },
      };
        ...
        for (;;) {
          ...
          if (nexttoken.id === "=") {
            ...
            if (peek(0).id === "=" && nexttoken.identifier) {
              error(
                "Constant {a} was not declared correctly.",
                nexttoken,
                nexttoken.value
              );
            }
    ...
    blockstmt("if", function () {
      ...
      expression(20);
      if (nexttoken.id === "=") {
        if (!option.boss) warning("Assignment in conditional expression");
        advance("=");
        expression(20);
      }
      advance(")", t);
