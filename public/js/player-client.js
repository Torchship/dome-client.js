...
function BarGraph(ctx) {
  var startArr,
    ...
    draw = function (arr) {
      var barWidth,
        ...
        maxBarHeight,
        i,
        numOfBars = arr.length,
        graphAreaWidth = self.width,
        graphAreaHeight = self.height;
      (ctx.canvas.width === self.width && ctx.canvas.height === self.height) ||
        ((ctx.canvas.width = self.width), (ctx.canvas.height = self.height)),
        ctx.clearRect(0, 0, self.width, self.height),
        (barWidth =
          self.fixedBarWidth || graphAreaWidth / numOfBars - 2 * self.margin),
        (maxBarHeight = graphAreaHeight - 1);
      var largestValue = 0;
      for (i = 0; i < arr.length; i += 1)
        arr[i] > largestValue && (largestValue = arr[i]);
      for (i = 0; i < arr.length; i += 1)
        (ratio = self.maxValue
          ? arr[i] / self.maxValue
          : arr[i] / largestValue),
  ...
  (this.width = 300),
    ...
    (this.update = function (newArr) {
      self.curArr.length !== newArr.length
        ? ((self.curArr = newArr), draw(newArr))
        : ((startArr = self.curArr), (endArr = newArr), looping || loop());
    });
}
Date.prototype.formatDate = function (format) {
  ...
  return (
    ...
    format.indexOf("t") > -1 &&
      (format =
        hours > 11 ? format.replace("t", "pm") : format.replace("t", "am")),
    format.indexOf("HH") > -1 &&
    ...
    format.indexOf("ss") > -1 &&
      (format = format.replace(
        "ss",
        seconds.toString().replace(/^(\d)$/, "0$1")
      )),
    format
  );
};
...
(dome.weakBrowser = function () {
  ...
  (dome.readPreferences = function () {
    ...
    if (options) {
      if (
        (-1 != options.indexOf("cs=false") &&
          (preferences.commandSuggestions = !1),
        -1 != options.indexOf("su=false") && (preferences.shortenUrls = !1),
        -1 != options.indexOf("cw=true") && (preferences.channelWindows = !0),
        -1 != options.indexOf("gm=true") && (preferences.godMode = !0),
        -1 != options.indexOf("pd=false") && (preferences.playDing = !1),
        -1 != options.indexOf("le=true") && (preferences.localEcho = !0),
        -1 != options.indexOf("iv=true") && (preferences.imagePreview = !0),
        -1 != options.indexOf("as=long")
          ? (preferences.autoScroll = "long")
          : -1 != options.indexOf("as=none") &&
            (preferences.autoScroll = "none"),
        -1 != (ofIndex = options.indexOf("of=")))
      ) {
        var rest = (of = options.substr(ofIndex));
        if (
          (-1 != (n = rest.indexOf("&")) && (of = rest.substr(0, n)),
          of.length > 3)
        ) {
      ...
      if (-1 != (etIndex = options.indexOf("et="))) {
        ...
        if (
          (-1 != (n = rest.indexOf("&")) && (et = rest.substr(0, n)),
          et.length > 3)
        ) {
      ...
      if (-1 != (clIndex = options.indexOf("cl="))) {
        ...
        if (
          (-1 != (n = rest.indexOf("&")) && (cl = rest.substr(0, n)),
          cl.length > 3)
        ) {
          var colorset = cl.substr(3);
          _.contains(COLORSET_CHOICES, colorset) &&
            (preferences.colorSet = colorset);
        }
      }
      if (-1 != (pbIndex = options.indexOf("pb="))) {
        var rest = (pb = options.substr(pbIndex));
        -1 != (n = rest.indexOf("&")) && (pb = rest.substr(0, n)),
          (pb = parseInt(pb)),
          pb > 0 && (preferences.performanceBuffer = pb);
      }
      -1 != options.indexOf("to=true") && (preferences.transparentOverlay = !0),
        -1 != options.indexOf("bs=false") && (preferences.broadSearch = !1);
    }
    return preferences;
  });
var PREFERENCE_ENUM = {
...
var CLIENT_OPTION_NAME_ERROR =
    "Unknown @client-option specified, check @client-options\n",
  ...
  translateClientOptionName = function (optionName) {
    return null != PREFERENCE_ENUM[optionName]
      ? PREFERENCE_ENUM[optionName].name
      : optionName;
  },
  ...
  setClientOption = function (optionName, optionValue) {
    ...
    "true" == optionValue
      ? (optionValue = !0)
      : "false" == optionValue && (optionValue = !1);
    var validValues = PREFERENCE_ENUM[optionName].valid || [!0, !1];
    if (!_.contains(validValues, optionValue))
      return dome.buffer.append(
        CLIENT_OPTION_VALUE_ERROR + validValues.toString() + "\n"
      );
    if (
      (clientOptions &&
      ...
      if (
        (dome.buffer.append(
          "changing @client-option " + optionName + " to " + optionValue + "\n"
        ),
        "colorSet" == optionName &&
          (dome.buffer.removeClass("colorset-" + dome.preferences.colorSet),
          dome.inputReader.removeClass(
            "colorset-" + dome.preferences.colorSet
          )),
        "lineBufferFont" == optionName &&
          dome.buffer.removeClass(dome.preferences.lineBufferFont + "Text"),
        "transparentOverlay" == optionName)
      ) {
        var ac = $(".ui-autocomplete");
        null != ac &&
          ac.removeClass(
            dome.preferences.transparentOverlay
      ...
      if (
        ((dome.preferences[optionName] = optionValue),
        "lineBufferFont" == optionName &&
          dome.buffer.addClass(dome.preferences.lineBufferFont + "Text"),
        "colorSet" == optionName &&
          "normal" != dome.preferences.colorSet &&
      ...
      "broadSearch" == optionName &&
        dome.preferences.commandSuggestions &&
        (dome.inputReader && dome.inputReader.commandSuggestions("destroy"),
        dome.autoComplete &&
          (dome.autoComplete(),
          dome.setupAutoComplete(dome.inputReader, dome.userType))),
        "commandSuggestions" == optionName &&
          (dome.preferences.commandSuggestions
            ? dome.autoComplete &&
              (dome.autoComplete(),
              dome.setupAutoComplete(dome.inputReader, dome.userType))
            : dome.inputReader &&
              dome.inputReader.commandSuggestions("destroy")),
        "localEcho" == optionName &&
          dome.setEchoButton(dome.preferences.localEcho),
        "imagePreview" == optionName &&
          dome.setImagesButton(dome.preferences.imagePreview);
    }
  };
(dome.parseClientOptionCommand = function (command) {
  if ((console.log(command), "@client-options" == command))
  ...
  (dome.setupInputReader = function () {
    $(document).keydown(function (e) {
      if (190 == e.keyCode && (e.ctrlKey || e.metaKey))
        return void dome.onToggleAutoScroll();
      if (191 == e.keyCode && (e.ctrlKey || e.metaKey))
        return void dome.inputReader.focus();
    ...
    if (dome.inputReader) {
      ...
      inputReader.on("keydown", function (event) {
        ...
        return 40 == event.which && commandPointer < commandBuffer.length - 1
          ? ((commandPointer =
              (commandPointer + 1 > commandBuffer.length ? 0 : commandPointer) +
              1),
            inputReader.val(commandBuffer[commandPointer]),
            event.preventDefault(),
            !1)
          : 40 == event.which && commandPointer >= commandBuffer.length - 1
          ? ((commandPointer = commandBuffer.length),
            inputReader.val() == lastInput && "" != inputReader.val()
              ? ((commandBuffer[commandBuffer.length] = inputReader.val()),
                commandBuffer.length > 2e3 && commandBuffer.shift(),
                (commandPointer = commandBuffer.length),
            ...
            return (
              socket.emit("input", command, function (state) {
                dome.preferences.localEcho &&
                  dome.buffer.append(
                    '<span class="input-echo">&gt;' + command + "</span>\n"
                  ),
                  dome.setFadeText &&
                    dome.statusDisplay &&
                    dome.setFadeText(
                      dome.statusDisplay,
                      state.status && 0 == state.status.indexOf("command sent")
                        ? "SENT"
                        : state.status,
                      !1
                    ),
                  command &&
                    0 == command.indexOf("@client-option") &&
                    dome.parseClientOptionCommand &&
                    dome.parseClientOptionCommand(command);
              }),
              (commandBuffer[commandBuffer.length] = inputReader.val()),
              commandBuffer.length > 2e3 && commandBuffer.shift(),
              (commandPointer = commandBuffer.length),
  ...
  (dome.setupWindowHandlers = function () {
    ...
    var onBlurHandler = function () {
      ...
      onResizeHandler = function () {
        dome.client.css("height", window.innerHeight + "px"),
          dome.buffer.css(
            "height",
            window.innerHeight - defaultHeightOffset + "px"
          );
      },
      ...
      alertTitle = function () {
        titleAlerted
          ? ((document.title = dome.titleBarText), (titleAlerted = !1))
          : ((document.title = "!! " + dome.titleBarText), (titleAlerted = !0));
      };
    ...
    winJQ.on("focus", onFocusHandler),
      ...
      iOS || winJQ.on("resize", onResizeHandler),
  ...
  (dome.setupSocket = function () {
    var onDisconnectedHandler = function () {
        console.log("disconnected"),
          ...
          dome.activeEditor && (dome.activeEditor.readingContent = !1),
          dome.setFadeText &&
            dome.statusDisplay &&
            dome.setFadeText(dome.statusDisplay, "DISCONNECTED", !0),
      ...
      onConnectedHandler = function () {
        dome.socketState == SOCKET_STATE_ENUM.DISCONNECTED &&
          onReconnectHandler(),
          (dome.socketState = SOCKET_STATE_ENUM.CONNECTED),
          dome.inputReader && dome.inputReader[0].focus(),
          dome.setFadeText &&
            dome.statusDisplay &&
            dome.setFadeText(dome.statusDisplay, "CONNECTED"),
          initialCommand ||
            setTimeout(function () {
              var cmd;
              if ((cmd = store.get("dc-initial-command")))
                ...
                (dome.alert.pattern = new RegExp(who)),
                  dome.setWindowTitle &&
                    dome.setWindowTitle(
                      who + " | " + gameName + " | " + poweredBy
                    ),
                  dome.socket.emit("input", cmd, function () {});
              }
              dome.preferences.shortenUrls &&
    ...
    return (
      (socket = io.connect(
        "https:" == document.location.protocol ? socketUrlSSL : socketUrl,
        {
  ...
  (dome.setupEditorSupport = function () {
    (window.getSocket = function () {
      ...
      (dome.makeEditor = function (editor) {
        var editWindow = null;
        if (
          _.has(dome.spawned, editor.editorName) &&
          null != dome.spawned[editor.editorName] &&
          ((editWindow = dome.spawned[editor.editorName]),
          editWindow.focus(),
          !editWindow.confirm(
            "Replace existing editor of the same name? You may have active edits."
          ))
        )
          return null;
        var type = "basic-readonly";
        editor.uploadCommand &&
          (type =
            -1 != editor.uploadCommand.indexOf("@program") ? "verb" : "basic"),
        ...
        var editorURL =
          ...
          type +
          "/?et=" +
          dome.preferences.edittheme +
          "&ts=" +
          new Date().getTime();
        if (null != editWindow && _.has(editWindow, "updateEditor"))
            ...
            if (dome.spawned.hasOwnProperty(title)) {
              ...
              null != editWin &&
                ...
                (listHTML +=
                  '<a data-editor="' +
                  title +
                  '" title="close editor" href="javascript:void(0);">'),
                (listHTML +=
                  '<i data-editor="' +
                  title +
                  '" class="glyph-button-close"></i></a></li>'));
            }
    ...
    null != dome.editorListView &&
      dome.editorListView.on("click", function (e) {
        if (e.currentTarget) {
          ...
          editorListClicked(
            editorName,
            "I" != e.target.tagName && "A" != e.target.tagName
              ? "zoom"
              : "close"
          );
        }
      ...
      (dome.editorClosed = function (editorName) {
        _.has(dome.spawned, editorName) &&
          (delete dome.spawned[editorName], dome.updateEditorListView());
      });
  }),
    ...
    (dome.onToggleAutoScroll = function (event) {
      ...
      dome.pauseBuffer
        ...
        : ((dome.pauseBuffer = !0),
          ...
          button.html(
            '<i class="icon-play icon-white" aria-hidden="true"></i><span class="hidden-xs">RESUME SCROLL</span>'
          ),
      ...
      "dbl" == dome.preferences.autoScroll
        ...
        ? (dome.buffer.on("mousedown", function (event) {
          ...
          dome.buffer.on("mouseup", function (event) {
            null != longClickProc && window.clearTimeout(longClickProc),
              (longClickProc = null);
          }))
  ...
  (dome.setupButtons = function () {
    (dome.setImagesButton = function (showImages) {
      dome.imagesButton &&
        (dome.imagesButton.html(
          showImages
            ? '<i class="icon-eye-close icon-white" aria-hidden="true"></i><span class="hidden-xs">NO IMAGES</span>'
            : '<i class="icon-eye-open icon-white" aria-hidden="true"></i><span class="hidden-xs">IMAGES</span>'
        ),
        showImages || $("I.icon-chevron-down", dome.buffer).trigger("click"));
    }),
      dome.imagesButton &&
      ...
      (dome.setEchoButton = function (showEcho) {
        dome.echoButton &&
          dome.echoButton.html(
            showEcho
              ? '<i class="icon-volume-off icon-white" aria-hidden="true"></i><span class="hidden-xs">HIDE ECHO</span>'
        ...
        dome.closeAllButton.on("click", function () {
          if (dome.spawned)
            for (var i = 0; i < dome.spawned.length; i++)
              dome.spawned[i] && dome.spawned[i].close();
          if (dome.channels)
            for (var i = 0; i < dome.channels.length; i++)
              dome.channels[i].window && dome.channels[i].window.close();
          window.close();
        }),
      (dome.attachImage = function (jqElem, imageId, url) {
        ...
        if (isVideo)
          (segment +=
            '<video id="' +
            imageId +
            '" loop muted autoplay class="shown-image" style="max-width: 75%">'),
            (segment +=
      ...
      (dome.toggleImage = function (elem, imageId, imageURL) {
        ...
        if (!control.length || !span.length)
          return void console.log(control, span, imageId);
        control.hasClass("icon-chevron-down")
          ? (control.removeClass("icon-chevron-down"),
            control.addClass("icon-chevron-up"),
            span.html(""))
  ...
  (dome.setupChannels = function () {
    (dome.channel = {}),
      ...
      (dome.channel.substitution = {
        type: "channel",
        pattern:
          /(\<span class\=\"ansi\-[A-Za-z]*\"\>)(\[[\-|\+]\])\[([-A-Za-z]*)\] (.*)(\<\/span\>)/g,
        replacement:
          '$1<span data-channel-name="$3" title="click to have future $3 go to its own window" class="filterable">$2[$3]</span> $4$5',
      }),
      dome.preferences.channelWindows &&
      ...
      (dome.channel.makeChannelWindow = function (name) {
        var channelWindow = window.open(
          "/channel/" + name + "/?ts=" + new Date().getTime(),
          "" + name,
          "width=640,height=480,resizeable,scrollbars"
        );
        return (
      ...
      (dome.channel.spawnChannel = function (name) {
        var channel = {};
        return (
          null != (channel = dome.channel.getChannel(name)) &&
          channel.window &&
          null != channel.window
            ? channel.window.focus()
            : null == channel || (channel.window && null != channel.window)
  ...
  (dome.autoComplete = function () {
    var commandArgumentPattern = new RegExp(/<[-A-Z a-z]+>/, "g"),
      prettyCommandArguments = function (unformattedString) {
        return _.reduce(
          unformattedString.match(commandArgumentPattern),
          function (out, commandArg) {
            return out.replace(
              commandArg,
              "<i>&lt;" +
                commandArg.substring(1, commandArg.length - 1) +
                "&gt;</i>"
            );
          },
          unformattedString
      ...
      (dome.setupAutoComplete = function (inputBuffer, userType) {
        null == inputBuffer ||
          (window.location.query &&
            -1 != window.location.query.indexOf("ac=no")) ||
          (dome.autoCommands.length > 0 &&
            inputBuffer.commandSuggestions("destroy"),
          $.ajax({
            url: "/ac/" + userType,
            dataType: "json",
            success: function (data) {
              (dome.autoCommands = _.reduce(
                data,
                function (out, line) {
                  var commandValue = line.trim(),
                    commandSearch = commandValue,
                    commandHelp =
                      '<div class="command-syntax">' + commandValue + "</div>",
                  ...
                  if (parts.length > 1) {
                    ...
                    commandParts.length > 1 && (commandValue = commandParts[0]),
                      (commandHelp =
                        '<div class="command-syntax">' +
                        prettyCommandArguments(commandSyntax) +
                        "</div>");
                    var commandInstruction = parts[1].trim();
                    if (
                      (dome.preferences.broadSearch &&
                        (commandSearch += commandInstruction),
                      (commandHelp +=
                        '<div class="command-instruction">' +
                        prettyCommandArguments(commandInstruction) +
                        "</div>"),
                ...
                inputBuffer.commandSuggestions({
                  ...
                  source: function (req, next) {
                    for (
                      var term = new RegExp(
                          (2 == req.term.length ? "^" : "") + req.term
                        ),
                        matches = [],
                        i = 0;
                      i < dome.autoCommands.length;
                      i++
                    ) {
                  ...
                  classes: {
                    "ui-autocomplete": dome.transparentOverlay
                      ? "ui-transparent-overlay"
                      : "ui-solid-overlay",
                  },
                });
  ...
  (dome.setupHealthCheck = function () {
    var showConnectionHelp = function (helpType) {
      ...
      troubleshootConnection = function (e) {
        ...
        return lastState == MOO_STATUS_ENUM.UNCHECKED
          ? ""
          : lastState == MOO_STATUS_ENUM.OK ||
            lastState == MOO_STATUS_ENUM.UNKNOWN
          ? "ETIMEOUT" == e.code && dome.gameHealth.cpu > 98
            ? (showConnectionHelp(MOO_STATUS_ENUM.SEVERE_LAG),
              "the moo is under heavy load and might not be able to respond in a timely manner")
            : "ENOTFOUND" == e.code || "ETIMEOUT" == e.code
            ? (showConnectionHelp(MOO_STATUS_ENUM.NETWORK_ISSUE),
              "unable to reach webclient server via socket, check your Internet connection")
            : "ECONNREFUSED" == e.code
            ? (showConnectionHelp("CHECK_FIREWALL"),
              "socket connection refused, behind a strict company or school firewall?")
            : (showConnectionHelp(MOO_STATUS_ENUM.NETWORK_ISSUE),
              "unexpected error while opening socket to webclient server: " +
                e.code)
          : (showConnectionHelp(lastState), dome.gameHealth.message);
      };
    (dome.onErrorHandler = function (e) {
      var msg = "";
      e &&
        (e.msg ? (msg = e.msg) : e.code && (msg = e.code),
        dome.socketState != SOCKET_STATE_ENUM.CONNECTED &&
          (msg = troubleshootConnection(e))),
        e && console && console.log(e),
        msg &&
          dome.statusDisplay &&
          dome.setFadeText(dome.statusDisplay, "ERROR: " + msg, !0);
    }),
      (dome.setFadeText = function (elem, msg) {
        (msg = msg.toUpperCase()),
          elem.stop(!0, !0).html(msg).animate(
            {
              opacity: "1",
            },
            500
    ...
    (dome.showGameHealth = function () {
      ...
      (dome.toggleGameHealth = function () {
        showingGameHealth && clickedOpen
          ? ((clickedOpen = !1), dome.hideGameHealth())
          : ((clickedOpen = !0), dome.showGameHealth());
      });
    ...
    var setGameHealthDisplay = function (health) {
      dome.gameHealth.push(health),
        dome.gameHealth.length > 100 && dome.gameHealth.shift();
      var globeClass = "ok";
      health.state != MOO_STATUS_ENUM.OK &&
      health.state != MOO_STATUS_ENUM.UNCHECKED
        ? (globeClass = "fatal")
        : health.cpu > 98 && (globeClass = "warn");
      var mem = (health.memory / 1024 / 1024).toFixed(2),
        details = health.message + "<br>";
      if (
        (health.cpu > 0 && (details += health.cpu + "% CPU consumption<br>"),
        health.memory > 0 && (details += mem + "MB RAM occupied<br>"),
        (details += health.users + " users connected<br>"),
        health.checked &&
          (details +=
            "Checked at " + new Date(health.checked).formatDate("hh:mm:ss t")),
        dome.healthDisplay.html(
          '<i class="globe globe-' +
            globeClass +
            '" onmouseover="dome.showGameHealth();" onmouseout="dome.hideGameHealth()"></i>'
        ),
        $(detailedMOOStatus).html(details),
        ("fatal" == globeClass ||
          (globeClass != lastGlobeClass &&
            dome.setFadeText &&
            dome.statusDisplay)) &&
          dome.setFadeText(
            dome.statusDisplay,
        ...
        for (
          var cpuValues = _.pluck(dome.gameHealth, "cpu"),
            memValues = _.pluck(dome.gameHealth, "memory"),
            userValues = _.pluck(dome.gameHealth, "users");
          cpuValues.length < 100;
    ...
    var updateMOOStatus = function () {
      0 != dome.preferences.performanceBuffer &&
        (dome.perfBufferFlag.attr(
          "title",
          "Scrollback limited to " +
            dome.preferences.performanceBuffer +
            " lines"
        ),
        ...
        $.get(
          ...
          err &&
            ...
            ("ENOTFOUND" == err.code
              ...
              : "ETIMEDOUT" == err.code
              ? (health.message =
                  "unable to reach webclient server after a reasonable time, server may be offline")
              : "ECONNREFUSED" == err.code
              ? ((health.state = MOO_STATUS_ENUM.NETWORK_ISSUE),
                (health.message =
                  "server connection refused, behind a strict company or school firewall?"))
              : (health.message =
                  "error while connecting to webclient server: " + err.code)),
  ...
  $(document).ready(function () {
    (dome = _.extend(dome, {
      ...
      "standard" != dome.preferences.lineBufferFont &&
        dome.buffer
          .removeClass("standardText")
          .addClass(dome.preferences.lineBufferFont + "Text"),
      "normal" != dome.preferences.colorSet &&
        dome.buffer.addClass("colorset-" + dome.preferences.colorSet),
      dome.setupChannels && dome.setupChannels(),
      dome.inputReader &&
        (dome.setupInputReader && dome.setupInputReader(),
        dome.preferences.commandSuggestions &&
          null != dome.autoComplete &&
          (dome.autoComplete(),
          dome.setupAutoComplete(dome.inputReader, dome.userType))),
      dome.setupWindowHandlers && dome.setupWindowHandlers(),
      dome.setupEditorSupport && dome.setupEditorSupport(),
      dome.setupAutoscroll && dome.setupAutoscroll(),
      dome.setupButtons && dome.setupButtons(),
      dome.setupHealthCheck && dome.setupHealthCheck(),
