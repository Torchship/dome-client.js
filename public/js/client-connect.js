var connectFunction = function (manual) {
  ...
  window.location =
    "/player-client/?game=sd&gh=" +
    host +
    "&gp=" +
    port +
    "&" +
    clientOptions.buildQueryString();
};
...
$(document).ready(function () {
  ...
  if (usernames.length > 0) {
    ...
    for (var i = 0; i < usernames.length; i++) {
      ...
      divider.before(
        '<li class="username" data-username="' + uname + '">' + uname + "</li>"
      );
    }
      ...
      userOptions.on("click", function (e) {
        ...
        if (clicked.hasClass("username")) {
          ...
          if (command == "purgeAll") {
            if (
              window.confirm(
                "You really want to delete all local user profiles?"
              )
            ) {
              store.purge();
  ...
  if ("https:" == document.location.protocol) {
    ...
    secureToggle.attr(
      "title",
      "You're about to connect in secure, encrypted mode. Click the lock icon to disable."
    );
    secureToggle.on("click", function () {
      if (window.confirm("Disable Encrypted Mode?")) {
        window.location =
          "http://" + document.location.hostname + document.location.pathname;
      }
    });
  } else {
    ...
    secureToggle.attr(
      "title",
      "You're about to connect in unencrypted mode. Click the open lock icon to switch to secure, encrypted mode."
    );
    secureToggle.on("click", function () {
      window.location =
        "https://" + document.location.hostname + document.location.pathname;
    });
  }
